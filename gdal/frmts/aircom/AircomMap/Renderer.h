...
public:
	...
	const Rectangle& getBoundingBox() const { return bounds; }
	...
	UniqueDataPtr readBlock(const Block& block, Rectangle& region) const;

	// Resamples a block region to the target resolution.
	// The region may be adjusted.
	UniqueDataPtr resample(const PixelType* data, Rectangle& region, int srcResolution) const;

	// Renders a region (in meters) in the target resolution into the result bitmap.
	void renderRegion(const PixelType* data, const Rectangle& region);
};
